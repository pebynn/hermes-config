...
version: "1.0.0"
author: "seaworld008"
source: "seaworld008/Commonly-used-high-value-skills"
...
- **技能版本升级**：在执行升级后，检查新版本是否悄悄增加了一些敏感权限或更改了核心逻辑。
...
**This skill has been superseded by `skill-security-auditor` (MCP 硬约束).**

Reasons:
- skill-security-auditor provides 4-layer detection (code execution / prompt injection / supply chain / file system)
- skill-security-auditor is now an MCP tool (mcp_skill-auditor.audit_skill) — hard constraint, cannot be skipped
- skill-vetter's functionality is fully covered

**Use `mcp_skill-auditor.audit_skill(path)` instead.**
