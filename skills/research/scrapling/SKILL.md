...
- **Node.js v24 EPIPE crash** (2026-05 实战): `StealthySession` 退出清理阶段在 Node.js v24.13.0 上报 `Error: write EPIPE`，pipe 断开导致脚本异常终止。page_action 能正常执行但退出时崩溃。**方案：降级 Node.js 或改用原生 Playwright + 手动 stealth。**
- **`real_chrome=True` 需要系统 Chrome**: 位于 `/opt/google/chrome/chrome`，Ubuntu 通常未装。没装时用 `real_chrome=False` 走 Playwright 自带 Chromium。
...
- **Node.js v24 EPIPE crash**: StealthyFetcher/StealthySession 底层使用 patchright (Playwright fork)，在 Node.js v24.13+ 环境下存在 EPIPE 管道断开问题。故障表现为页面操作可正常执行完成，但在退出/session清理阶段抛出 `Error: write EPIPE` 并超时（Node.js v24 事件循环兼容性）。**workaround**: 场景为持久化浏览器会话（如Cookie持久化登录）时，改用原生 `playwright.sync_api` + 手动 stealth 注入（`--disable-blink-features=AutomationControlled` + `add_init_script` 覆盖 `navigator.webdriver`）+ `context.storage_state()` 保存/恢复会话，效果等同于 scrapling 的 `user_data_dir`。scrapling 的 `Fetcher`/`FetcherSession`（纯 HTTP）不受影响
