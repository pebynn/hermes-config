---
author: unknown
description: any input (code, docs, papers, images) → knowledge graph → clustered
...
`mcp-graphify.py` 硬编码 `GRAPH_PATH`，Hermes 生态的实际图谱位于 `~/brain/graphify-out/graph.json`。

### MCP Server 内存缓存问题

`mcp-graphify.py` 使用模块级 `_graph_data` 变量缓存图谱数据。修改 `graph.json` 后（如 `merge-graphs` 合并或手动添加边），**MCP server 进程不会自动重载**——它持有旧缓存。

**症状**: `graph_find_path` 找不到新添加的边，但 `graph.json` 文件中边已存在。

**验证**: 用 CLI 直接查 `graph.json`：
```bash
graphify path "新节点A" "新节点B" --graph ~/brain/graphify-out/graph.json
```
CLI 每次读取文件 → 能看到新边。MCP server 读内存缓存 → 看不到。

**修复**: 重启 gateway → 新会话 → MCP server 重新启动 → 重新加载 `graph.json` 到内存。也可清 `__pycache__`：
```bash
find ~/.hermes/mcp-servers/__pycache__ -delete
```

### MCP Server 内存缓存失效

⚠️ **高频陷阱**: MCP server 用模块级变量 `_graph_data` 缓存图谱。修改 graph.json 后，MCP 查询返回旧数据。CLI (`graphify path`) 和 MCP 结果不一致是典型症状。

详见: `references/mcp-caching-pitfall.md`

快速诊断: `graphify path "A" "B"` 有结果但 `mcp_graphify_graph_find_path` 返回 "No path" → 缓存未刷新。新 Session 自动修复。

### MCP Server 内存缓存

`mcp-graphify.py` 模块级 `_graph_data` 变量缓存图数据。直接修改 `graph.json` 后已运行的 MCP server 进程不会自动重载，`graph_search`/`graph_find_path` 返回旧数据。

症状: graph.json 确认已更新，但 MCP 查询返回旧结果或找不到新边。

修复: 重启 gateway 或重启 Hermes 会话。新 session 初始化时会重新 `load_graph()`。

验证: `graphify path "A" "B" --graph <path>` (CLI 直读文件，可绕过缓存)。CLI 确认边存在即可信任下次 MCP 会话。

### Hindsight 误判陷阱

错误日志中 `ModuleNotFoundError: No module named 'hindsight'` 可能是旧 gateway 实例残留（路径包含 `tools/hermes-agent-webapi/`）。当前 gateway 使用 `~/.hermes/hermes-agent/venv`。验证方法：`~/.hermes/hermes-agent/venv/bin/python -c "import hindsight; print(hindsight.__path__)"`，而非检查日志文件。Hindsight 本地安装路径为 `/home/pebynn/hindsight/`（pip install -e）。

### 0 边问题

若 `graph_stats` 返回 nodes>0 但 edges=0，说明 AST 抽取未运行或语义抽取的关系未被写入。重跑 graphify 即可。

### 命名空间隔离 (brain:: vs .hermes::)

⚠️ **归档级陷阱**: Hermes 知识图谱存在两个独立命名空间——`brain::` 和 `.hermes::`。两者包含重叠的域文档节点（如两个空间都有 `hermes_writing-domain`），但 **图遍历路径完全隔离**。`graph_find_path` 在跨命名空间的节点间返回 "No path"。

**成因**: graphify-daily cron (e1917ae814df) 对 `~/brain/soul/` 爬取生成 `brain::` 节点，另一次爬取对 `~/.hermes/profiles/` + `~/.hermes/SOUL.md` 生成 `.hermes::` 节点。相同概念（数据铁律、域规则）在两个空间各自存在但无桥接边。

**症状**: 
- `graph_search("数据铁律")` → 在 brain:: 下找到节点（含22条边，连接丰富）
- `graph_search(".hermes::")` → 在 .hermes:: 下找到同名节点（仅1条自引用边）
- `graph_find_path(source="brain::concept_数据铁律", target=".hermes::hermes_writing-domain")` → "No path"

**影响**: graph_search 跨命名空间全局搜索正常，但图遍历(find_path/社区检测)只能在单个命名空间内工作。跨域推理返回空结果。

**快速诊断**:
```bash
# 检查两个空间的域节点是否都有边
graphify explain ".hermes::hermes_writing-domain" → edges=1（自引用孤岛）
graphify explain "brain::brain_writing_domain" → edges=22（连接丰富）
```

**长期修复**: 在 graphify 合并阶段检测跨命名空间同名概念，自动建立 `equivalent_to` 桥接边。短期可手动在 graph.json 中添加桥接边。

**完整审计方法**: 图谱连接性四步诊断法（域节点发现→跨域路径测试→概念定位→修复分级），详见 `references/graph-connectivity-audit.md`。

### Terminal 命令误判背景化

`terminal()` 有时会误判 Python one-liner 为 `&` 后台命令，报错 "Foreground command uses '&' backgrounding"。即使命令不含 `&` 也可能触发（疑似 Hercules 后端正则过宽）。

修复：将 Python 代码写入临时 `.py` 文件 → 直接运行该文件 → 完成后清理。

```
# 错误写法（可能被误判）：
terminal(command="python3 -c \"...长多行代码...\"")

# 正确写法：
write_file(content="...代码...", path=".graphify_stepN.py")
terminal(command="python3 .graphify_stepN.py")
# 完成后 rm .graphify_stepN.py
```

### Cron 执行方式

graphify 通过 `/graphify` slash command 运行（由 skill 触发），不是 `graphify` CLI 二进制。Cron prompt 中应写 `/graphify --update <path>`。
