...
| **stop_once 组合止损仅触发一次 — 后续回撤无保护 (2026-05-14 发现+修复)** | `stop_once = True` 在首次触发组合止损后永久置True，后续任何深度回撤均不再拦截。全期回测可致-54.6%回撤。根因：`ps_hit = (dd <= -ps and not stop_once)` → `if ps_hit: stop_once = True` | 改为冷却机制：`ps_cooldown` 替代 `stop_once`。触发后20天冷却，期满可再次触发。冷却期内禁止买入。2025回测验证：ps=10%→年化747%/回撤-10.6%，ps=15%→年化-0.8%/回撤-18.5%。10%更优。详见 `references/portfolio-stop-cooldown-fix.md` |
| **动量策略含688/92股票池严重拖累表现 (2026-05-14 发现+修复)** | 科创板(688)/北交所(92)股票极端波动+低流动性，动量排名噪声污染。含全集时年化342%/夏普4.89/回撤-10.2%；排除后年化1378%/夏普8.88/回撤-6.8%。股票池减少741只(4769→4028)，组合止损从6次→0次 | 在universe构建时添加前缀过滤：`not (c.startswith('688') or c.startswith('92'))`。ST通过 `excluded_stocks.json` + `MIN_AMOUNT_V2=5000万` 自然排除。详见 `references/universe-filter-688-92-st.md` |
| **主板浓缩池放大年份不稳定性 (2026-05-14 发现)** | 仅A股主板(60x+000/001)1965只: 2026年化1638% (优于全市场1378%), 但2025年化-18.2% (vs全市场9.9%)。浓缩池=对市场风格轮动赌注加大。 | 默认用全市场-688/92(两年都正)。主板仅适合已确认主板动量行情时临时切换。详见 `references/universe-filter-board-restriction.md` |
| **Python函数内import语句导致UnboundLocalError (2026-05-14)** | 在函数体内写 `if cond: import json` — Python解析器看到函数内的import语句后，将`json`标记为局部变量，屏蔽模块级import。若条件不满足导致import未执行，后续任何`json.xxx`调用报UnboundLocalError | 避免在函数体内局部import。如需条件导入，用`importlib`或在函数外统一import。函数内import语句即使不执行也会绑定名称。 |
| **回测"今日因子→今日开盘"存在前视偏差 (2026-05-14 发现+修复)** | 将xr从 `dates[ci-1]`（昨日）改为 `cd`（当日）后，若直接当日开盘买入，则排名使用的因子数据在真实时间线中尚未可知（收盘后才能计算因子） | 采用 `pending_buys` 延迟执行模式：当日因子排名 → 填入 pending_buys 列表 → 次日开盘执行买入。T+1自然保护当日买入不可卖。详见 `references/backtest-deferred-buy-pattern.md` |
| **市值50-500亿过滤拖累2026动量表现 (2026-05-14 发现)** | 全市场-688/92池(4028只): 年化1378%/回撤-6.8%。加市值过滤后(2586只): 年化743%/回撤-8.6%。被踢掉的<50亿小票在2026年动量极强，是alpha主力。市值过滤在动量行情中反而有害 | 市值过滤更适合基本面多因子策略(关注估值合理性)，不适合纯动量策略(小票动量弹性大)。用 `stock-sdk MCP get_all_a_share_quotes` → 解析 `totalMarketCap`(亿元) → 筛选区间 → 保存JSON → 回测加载。详见 `references/market-cap-filter-pipeline.md` |
| **信号生成器与回测逻辑对齐陷阱 (2026-05-14 发现+修复)** | signal_generator.py 与 strategy_v2.py 出现三处不一致: (1) `build_universe` 未过滤688/92/ST (回测有); (2) 买入动量过滤用 `ret_5d` 而回测用 `mom`(ret_10d); (3) 摘要显示写\"5日动量\"实为10日。不一致→信号生成器的买入信号与回测Top排名不同 | 任何回测改动后同步检查信号生成器: 股票池过滤器、因子字段名、买入执行时机、排名函数参数。详见 `references/signal-generator-backtest-alignment.md` |
| **pending_buys延迟在市场年份间表现天差地别 (2026-05-14 发现)** | 策略A v2实测: 2025年延迟版年化9.9% vs 当日收盘版50.7%(延迟吞80%alpha)；2026年延迟版1378% vs 当日收盘版1202%(延迟多吃176%年化)。延迟在动量衰减快的年份致命，在动量持续的年份反而加分。浓缩池(主板/市值)放大不稳定性 | pipeline必须双测: 先跑当日收盘baseline → 再加pending_buys对比。差距>3x→策略对执行时机极度敏感,实盘风险大。详见 `references/backtest-deferred-buy-pattern.md` |
| **全年回测并行OOM (2026-05-14)** | 两个全年(243天)回测并行运行，每进程~1.5GB，系统空闲仅2.5GB → OOM kill。两个进程都在"1/5 数据..."阶段被杀 | 全年回测必须串行。2026(83天)可并行，但全年(≥200天)不可。 |
| **Python默认参数在函数定义时绑定 — 模块级Override无效 (2026-05-14 发现)** | `def run_backtest_v2(ps=PORTFOLIO_STOP_V2, ...)` 在函数定义时将 `PORTFOLIO_STOP_V2=0.15` 绑定为默认值。之后 `strategy_v2.PORTFOLIO_STOP_V2 = 0.10` 再调 `run_backtest_v2()` 仍用旧值0.15 | 必须显式传参：`run_backtest_v2(ps=0.10)`。模块级变量override对已定义的函数默认参数无效。回测结果完全相同时先怀疑这个。 |
| **no_agent cron脚本路径限制 (2026-05-14)** | `cronjob create --script` 的路径必须是 `~/.hermes/scripts/` 下的文件名，不支持绝对路径。提交绝对路径报 `Script path must be relative to ~/.hermes/scripts/` | 在 `~/.hermes/scripts/` 创建wrapper shell脚本，内部cd到目标目录再exec。详见 `references/signal-generator-pipeline.md` |
| **backtest函数参数覆盖陷阱 (2026-05-14 归档)** | `run_backtest(ps=0.10)` vs `strategy.PS=0.10; run_backtest()` 两套结果完全一致时，第一个怀疑就是默认参数绑定。验证方法：看print输出中的参数值是否变化 | 参数测试一律显式传参，不要依赖模块级变量override |
...
- **信号生成器管线**: `references/signal-generator-pipeline.md` — 回测→信号生成器改造方案、持仓状态持久化、cron集成
